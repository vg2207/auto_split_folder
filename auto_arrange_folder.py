    ...
    if user_input_folder is not None:
        ...
        if submit_button_clicked :
            ...
            for i in range(len(lst)):
                ...
                st.write(glob.glob(os.path.join(os.getcwd(),os.path.splitext(user_input_folder.name)[0],'*pdf'))[i-1])
                # shutil.copy(glob.glob(os.path.join(target_path,'*pdf'))[i], path_to_save)
                shutil.copy(glob.glob(os.path.join(os.getcwd(),os.path.splitext(user_input_folder.name)[0],'*pdf'))[i-1], path_to_save)
